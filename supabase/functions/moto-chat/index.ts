...
let cache: { places: PlaceRow[]; courses: CourseRow[]; exp: number } | null = null;
...
async function loadData() {
  ...
  cache = { places, courses, exp: Date.now() + 5 * 60 * 1000 };
  return cache;
}
...
function buildDataset(
  ...
  const placeLines = places.map((p) => {
    const dist = loc ? ` (${km(loc.latitude, loc.longitude, p.latitude, p.longitude)}km)` : '';
    const tags = p.tags?.length ? ` [${p.tags.join(',')}]` : '';
    return `- id:${p.id} | ${p.name} | ${CATEGORY_LABELS[p.category] ?? p.category} | ${p.address}${dist}${tags} | ${p.description ?? ''}`;
  });
  const courseLines = courses.map((c) => {
    const start = c.coordinates?.[0];
    const dist = loc && start ? ` (출발지까지 ${km(loc.latitude, loc.longitude, start[1], start[0])}km)` : '';
    const tags = c.tags?.length ? ` [${c.tags.join(',')}]` : '';
    return `- id:${c.id} | ${c.name} | ${c.distance}km ${c.duration}분 ${c.difficulty}${dist}${tags} | ${c.description ?? ''}`;
  });
...
Deno.serve(async (req) => {
  ...
  try {
    const { places, courses } = await loadData();
    const loc =
      body.location &&
    ...
    const system = `너는 "모토맵"(한국 오토바이 라이더용 지도 앱)의 추천 도우미다.
...
- 사용자 위치가 있으면 거리(각 항목에 표기됨)를 고려해라. "근처"를 물으면 가까운 순으로.
- 라이딩·장소·코스와 무관한 주제(정치, 숙제, 일반 상식 등)는 정중히 거절하고 라이딩 추천으로 화제를 돌려라.
${loc ? `\n사용자 현재 위치: ${loc.latitude.toFixed(4)}, ${loc.longitude.toFixed(4)} (각 항목의 거리는 이 위치 기준)` : '\n사용자 위치: 미제공 (위치 기반 질문이면 지역을 되물어라)'}

${buildDataset(places, courses, loc)}`;
    ...
    return json({ reply: out.reply ?? '', places: placeCards, courses: courseCards });
  } catch (e) {
