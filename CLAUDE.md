...
## 도메인 의사결정 기록
- 기능을 수정·추가·삭제하기 전에 **가장 먼저** `docs/domain-decisions/README.md`의 색인과 관련 도메인 문서를 읽는다. 코드 탐색과 구현 계획보다 이 검토를 우선한다.
- 활성 원칙뿐 아니라 과거 결정의 근거, 실패한 접근, 재검토 조건까지 확인하고 같은 시행착오를 반복하지 않는다.
- 의미 있는 결정이 추가·변경되거나 새로운 시행착오가 생기면 코드와 같은 논리 단위의 커밋에서 관련 문서를 갱신한다. 관련 문서가 없으면 새로 만들고 색인에 연결한다.
- 기존 기록을 삭제하거나 조용히 덮어쓰지 않는다. 결정이 바뀌면 이전 항목을 `대체됨`으로 표시하고 후속 결정을 연결한다.
...
- **naver-map 스택 아래 화면**: 다른 화면이 위에 push 되면 지도 뷰가 detach 되어 **카메라 명령을 버리고**, blur 중 커밋된 RN UI 도 화면에 반영되지 않는다(2026-08 실기기 HUD 로 실증). "복귀 전에 카메라를 미리 옮겨두기" 계열(스토어 선행·직접 네이티브 호출·전환 지연·커튼)은 전부 불가능 — 이동은 복귀 후에 실행되므로 감추지 말고 `easing: 'Fly'` 애니메이션으로 보여주는 것이 정답이었다. 즉시 포커스가 필요하면 새 지도 인스턴스(place-preview 오버레이, initialCamera=목표)로 연다.
