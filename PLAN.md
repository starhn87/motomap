...
- [ ] **Universal Links** — 장소·코스·라이더 카드를 HTTPS 링크로 열고 미설치 시 App Store로 폴백
  - Associated Domains와 도메인 검증 파일이 필요해 네이티브 설정 변경이 발생한다.
- [ ] **공유 카드 이미지 내보내기** — 라이더 패스포트·주행 회고를 이미지 파일로 저장·공유
  - 우선 OTA에서는 텍스트·링크 공유로 사용성을 검증하고, 로컬 뷰 캡처가 필요할 때 네이티브 모듈을 추가한다.
- [ ] **햅틱 피드백** — 탭바·즐겨찾기·제보 완료·마커 선택·전방 위험 경고
  - 현재 `expo-haptics`가 바이너리에 없어 OTA로 추가할 수 없다.
