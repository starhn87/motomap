...
라이딩 맛집·카페·뷰포인트·주유소·정비소를 지도에서 찾고, 리뷰·평점·코스 공유로 라이더끼리 정보를 나눕니다. 출발 전에는 기상청 예보 기반 라이딩 날씨로 비 소식을 확인합니다. **App Store 정식 출시.**

## 📱 스크린샷

| 지도 · 장소 탐색 | 라이딩 날씨 | 어디든 장소 카드 |
|:---:|:---:|:---:|
...
- 📍 **어디든 장소 카드**: 지도의 가게 아이콘·이름을 탭하면 등록되지 않은 곳도 바로 카드가 뜨고 길안내·제보로 이어집니다 (네이티브 심벌 탭, `patches/`)
- 🔎 **통합 검색**: 등록 장소·코스에 더해 카카오 로컬 일반 장소까지 검색해 길안내. 집·회사를 저장하면 원터치 길안내
- 🌦️ **라이딩 날씨**: 기상청 단기예보 기반 라이딩 점수와 시간대별 강수확률. 길안내 전 경로의 비 소식 경고
- ⛽ **실시간 유가**: 오피넷 연동 주변 주유소 가격, 최저가 표시
- 🙋 **장소 제보**: 크라우드소싱 장소 추가 (주소 검색, 사진 포함). AI 사전 심사 후 승인·반려 결과를 푸시와 인앱 알림으로 받습니다
- ⭐ **리뷰·평점**: 사진 스와이프, 수정·삭제, 리뷰어 바이크 뱃지
- 🛣️ **라이딩 코스**: 추천 코스 목록 + 지금이 제철 뱃지 + 경로 미리보기
- 🤖 **AI 추천**: 대화로 코스·장소를 골라주는 챗
- 🏍️ **내 바이크**: 기종 자동완성 1,040종 ([moto-kr](https://github.com/starhn87/moto-kr) 오픈소스 API)
...
| `EXPO_PUBLIC_KAKAO_REST_API_KEY` | 카카오 로컬 REST 키 (장소 검색·지오코딩) |
| `EXPO_PUBLIC_SENTRY_DSN` | Sentry DSN (선택) |
| `SUPABASE_SERVICE_ROLE_KEY` | 시드 스크립트 전용 (선택) |

> 로컬 EAS 명령(`eas build`/`submit`)은 `.env`를 자동 로드하지 않으므로, 클라우드 빌드용 키는 EAS 환경 변수에 등록되어 있습니다 (`eas env:list --environment production`). 날씨·유가·AI 키는 앱이 아닌 Supabase Edge Function secrets에 있습니다.
