...
// 시드 소스마다 정식형("강원특별자치도")과 축약형("강원")이 섞여 들어와, 같은
// 지역이 두 이름으로 갈리면 검색 결과에 나란히 보이고 지역 칩도 쪼개진다.
// 축약형이 한 줄 표시와 칩에 유리해 기준으로 삼는다. 카카오 로컬은 정식형을
// 주므로 앱에서 lib/region.ts 의 normalizeSido 가 같은 규칙으로 맞춘다.
//
