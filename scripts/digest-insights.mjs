...
- 완주율의 "비정상 종료"는 안내 중 앱 강제 종료·크래시를 다음 실행 때 정산한 것 — 계측 유실이 아니라 이미 보정된 수치다. 다만 이 정산은 앱 업데이트 이후 데이터부터 잡힌다
- "우리 DB에 없던 검색어"는 카카오에는 있는데 등록 장소 DB에 없던 검색 — 장소 시드·제보의 우선순위 신호
- "라이더가 다녀왔는데 아직 등록 안 된 곳"은 길안내로 실제 도착한 미등록 장소(누적) — 검색만 한 것보다 강한 등록 후보다
