...
8. 운영 중단 뒤 재개하는 릴리스는 일반 배포보다 `OPS-002`를 우선한다. 활성 production OTA나
   운영 재개 푸시만으로 서비스를 먼저 열지 않고, 재개 연결 화면 → 1.2.8 스토어 가용성 →
   백엔드·공급자 복구 검증 → 공개 상태 `active` 전환 → 운영 공지 순서를 지킨다.
