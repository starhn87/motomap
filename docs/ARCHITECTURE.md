...
| `/navi` | 길안내 버튼(장소·코스·길찾기) | 경로 미리보기 — 옵션(추천·시간·거리·큰길)별 경로 지도 표시 → KNSDK 안내 시작. 코스 안내가 아니면 상단 카드에서 출발지·경유지(최대 5)·도착지 편집·스왑 |
| `/directions` | 지도 탭 검색바 옆 버튼, 장소 상세 [출발] | 길찾기 — 출발지·도착지 검색(카카오 로컬)·스왑 → 미리보기 |
