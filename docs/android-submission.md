...
위치 권한(앱 사용 중), 네이버 Maven, 카카오 android 옵션)이 들어가 있다. 남은 일은
**빌드 → 외부 콘솔 등록 → Play 심사**다.

> ⚠️ 핵심 관문 — 네이버·카카오 콘솔에 **android 키해시/패키지 등록**(안 하면 빌드돼도 지도·내비가
> 안 뜸). **주행 기록 제거로 백그라운드 위치 Play 추가 심사(시연 영상 등)는 사라졌다** — 위치는
> "앱 사용 중"만 쓰므로 출시가 한결 단순하다.
...
- [ ] 지도 표시 + 마커 + 카메라 이동
- [ ] 위치 권한 허용 → 내 위치 표시
- [ ] 제보 주소 검색 (카카오 로컬 — 상호/주소 입력 → 결과 선택 → 좌표 채움)
...
수집 항목 선언:
- **위치**(정밀, **앱 사용 중만** — 백그라운드 없음), **사진/동영상**, **이메일·계정**, **사용자 콘텐츠**(리뷰·제보)
- 전송 중 암호화, 사용자 **삭제 요청 가능**(앱 내 계정 탈퇴 기능 있음 — `delete_my_account`)

> ✅ **백그라운드 위치 심사 없음** — `ACCESS_BACKGROUND_LOCATION`을 쓰지 않으므로(주행 기록 제거),
> 예전이라면 필요했을 Play 백그라운드 위치 선언·시연 영상 단계가 통째로 빠진다.

## 8. production 빌드(aab) + 제출
...
- `version`은 iOS와 공유(`1.1.1`). android `versionCode`는 EAS `appVersionSource: remote` +
